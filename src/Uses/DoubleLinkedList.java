...
public class DoubleLinkedList<T> implements Iterable<T> {
    ...
    /**
     * Gives a array of the elementes in the list in a inverted way
     *
     * @param rear last node in the list
     * @return a array of the elementes in the list in a inverted way
     */
    public T[] invertedContent(DoubleNode rear) {
        T[] arr = (T[]) new Object[size()];
        return InvertedContentAUX(arr, rear, 0);

    }

    private T[] InvertedContentAUX(T[] arr, DoubleNode node, int index) {
        arr[index] = (T) node.getElement();
        return (node.getPrevious() != null)
                ? (InvertedContentAUX(arr, node.getPrevious(), index + 1))
                : (arr);

    }

    /**
     * Replaces all occurs of one element for another.
     *
     * @param existingElement element to be found and changed
     * @param newElement element to change and introduce in the list
     */
    public void replace(T existingElement, T newElement) {
        replaceAUX(head, existingElement, newElement);
    }

    /**
     * 
     * @param node the first node to start looking for changes
     * @param existingElement element to be found and changed
     * @param newElement element to change and introduce in the list
     */
    private void replaceAUX(DoubleNode node, T existingElement, T newElement) {
        if (node != null) {
            if (existingElement.equals(node.getElement())) {
                node.setElement(newElement);
            }
            replaceAUX(node.getNext(), existingElement, newElement);
        }
    }
